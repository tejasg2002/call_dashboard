...
export default function WAFilters({ filters, setFilters, options, theme }) {
  ...
  const hasFilter = filters.templateName || filters.startDate || filters.endDate
  ...
  const inputBase = `appearance-none w-full rounded-xl border pl-9 pr-3 py-2.5 text-[13px] font-medium outline-none transition-all ${
    isDark
      ? 'bg-slate-800 border-slate-700 text-slate-200 hover:border-slate-600 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20'
      : 'bg-white border-slate-200 text-slate-700 hover:border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 shadow-sm'
  }`
  ...
  return (
    <div className="flex items-center gap-3 flex-wrap">
      ...
      {hasFilter && (
        <button
          onClick={() => setFilters({ templateName: '', eventType: '', startDate: '', endDate: '' })}
