    ...
    <div className={cn(
      ...
      <div className="px-5 pt-5 pb-3 space-y-3">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <input
            type="text"
            placeholder="Search phone or lead ID..."
            value={search}
      ...
      <div className="overflow-x-auto overflow-y-auto max-h-[520px] min-w-0">
        <table className="w-full min-w-[640px] text-[11px] table-fixed">
          <thead className={cn("sticky top-0 z-10", isDark ? "bg-slate-800" : "bg-slate-50")}>
            <tr>
              <th className={cn("w-[128px] px-4 py-2.5 text-left font-semibold uppercase tracking-wider text-[10px]", isDark ? "text-slate-400" : "text-slate-500")}>Phone</th>
                ...
                <tr key={u.phone} className="group">
                  <td colSpan={6} className="p-0">
                    <div
                      ...
                      className={cn(
                        "flex w-full min-w-0 items-center cursor-pointer transition-colors px-4 py-2.5 gap-1",
                        isExpanded
                    ...
                    >
                      <div className="w-[128px] shrink-0 min-w-0 overflow-hidden">
                        <span className={cn("block font-mono font-medium truncate", isDark ? "text-slate-200" : "text-slate-800")} title={mp(u.phone)}>{mp(u.phone)}</span>
                      </div>
                    ...
                    {isExpanded && (
                      <div className={cn("min-w-0 overflow-x-auto px-4 pb-3 pt-1", isDark ? "bg-slate-800/40" : "bg-slate-50/80")}>
                        <table className="w-full min-w-[480px] table-fixed text-[10px]">
                          <thead>
                            <tr>
                              <th className={cn("w-7 py-1.5 pr-2 text-left font-medium", isDark ? "text-slate-500" : "text-slate-400")}>#</th>
                              <th className={cn("w-[22%] py-1.5 pr-2 text-left font-medium", isDark ? "text-slate-500" : "text-slate-400")}>Template</th>
                              <th className={cn("w-[14%] py-1.5 pr-2 text-left font-medium", isDark ? "text-slate-500" : "text-slate-400")}>Button</th>
                              <th className={cn("w-[42%] min-w-0 py-1.5 pr-2 text-left font-medium", isDark ? "text-slate-500" : "text-slate-400")}>Link</th>
                          ...
                          <tbody>
                            {u.clicks.map((c, i) => (
                              <tr key={i} className={cn("border-t", isDark ? "border-slate-700/50" : "border-slate-200/50")}>
                                ...
                                <td className="w-[42%] min-w-0 py-1.5 pr-2 align-top">
                                  {c.link ? (
                                    <a
                                      href={c.link}
                                      target="_blank"
