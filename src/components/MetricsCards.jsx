...
const MetricsCards = ({ kpi, loading, dateLabel = 'All time', unified = false }) => {
  ...
  const analyzedCalls = kpi?.analyzedCalls ?? 0
  ...
  const unifiedCards = [
    ...
    },
    {
      ...
      sub: analyzedCalls > 0 ? `${interestedPct}% of analyzed · call analysis` : `${dateLabel} · call analysis`,
      accent: 'green',
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
    ...
    {
      label: 'Call Analysis Total',
      value: isEmpty ? '—' : analyzedCalls.toLocaleString('en-IN'),
      sub: `${dateLabel} · call analysis`,
      accent: 'amber',
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
  ...
  const defaultCards = [
    ...
    },
    {
      ...
      detail: !isEmpty && (
        <div className="mt-4">
          <div className="h-1.5 rounded-full bg-slate-100 dark:bg-slate-700/60 overflow-hidden">
            <div className="h-full bg-blue-500 rounded-full transition-all duration-700" style={{ width: `${scorePct}%` }} />
          </div>
          <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1.5">{scorePct}% of max</p>
        </div>
      ),
  ...
  const cards = unified ? unifiedCards : defaultCards
