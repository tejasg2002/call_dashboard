...
const PerformanceCards = ({ topPerformer, ownerStatsToday, ownerStatsMonth, dateLabel = 'All time' }) => {
  const formatOwner = (owner) =>
    owner.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())

  if (topPerformer) {
    const card = topPerformer
    return (
      <div className="grid grid-cols-1 gap-4 mb-6">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {formatOwner(card.owner)?.[0] || '?'}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">
                {formatOwner(card.owner)}
              </p>
              <p className="text-[11px] text-slate-400 dark:text-slate-500">
                {card.totalCalls} calls · avg score {card.avgScore}
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            <div className="text-center">
              <p className="text-lg font-bold font-mono text-slate-900 dark:text-white">{card.totalCalls}</p>
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] text-slate-400 dark:text-slate-500">
                {card.calls} calls · avg score {card.avgScore}
              </p>
            </div>
          </div>
