      ...
      <div className="overflow-y-auto max-h-[520px]">
        <table className="w-full text-[11px]">
          <thead className={cn('sticky top-0 z-10', isDark ? 'bg-slate-800' : 'bg-slate-50')}>
            <tr>
              <th className={cn('px-4 py-2.5 text-left font-semibold uppercase tracking-wider text-[10px]', isDark ? 'text-slate-400' : 'text-slate-500')}>Email</th>
              ...
              return (
                <tr key={u.email} className="group">
                  <td colSpan={5} className="p-0">
                    ...
                    >
                      <div className="w-[200px] min-w-[160px] shrink-0 pr-2">
                        <span className={cn('font-mono font-medium break-all text-[10px]', isDark ? 'text-slate-200' : 'text-slate-800')}>{me(u.email)}</span>
                      </div>
                        ...
                        <table className="w-full text-[10px]">
                          <thead>
                            <tr>
                              ...
                              <th className={cn('text-left py-1.5 font-medium', isDark ? 'text-slate-500' : 'text-slate-400')}>Subject</th>
                              <th className={cn('text-left py-1.5 font-medium', isDark ? 'text-slate-500' : 'text-slate-400')}>Link label</th>
                              <th className={cn('text-left py-1.5 font-medium', isDark ? 'text-slate-500' : 'text-slate-400')}>URL</th>
                          ...
                          <tbody>
                            {u.clicks.map((c, i) => (
                              <tr key={i} className={cn('border-t', isDark ? 'border-slate-700/50' : 'border-slate-200/50')}>
                                ...
                                <td className={cn('py-1.5 pr-2 font-medium max-w-[220px]', isDark ? 'text-slate-200' : 'text-slate-700')}>
                                  <span className="line-clamp-2" title={c.template || ''}>{c.template || '—'}</span>
                                  {c.templateId ? (
                                    <span className={cn('block text-[9px] mt-0.5', isDark ? 'text-slate-500' : 'text-slate-400')}>ID: {c.templateId}</span>
                                  ) : null}
                                </td>
                                ...
                                <td className={cn('py-1.5 whitespace-nowrap', isDark ? 'text-slate-400' : 'text-slate-500')}>
