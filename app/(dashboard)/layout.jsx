...
function WorkspaceDropdown({ isDark }) {
  ...
  const buSwitcherOptions = useMemo(() => {
    if (allowedBuWorkspaces === undefined) {
      return [
        { workspace: WA_WORKSPACE_MBA, label: 'MBA' },
        ...ANALYTICS_WA_DEFINITIONS,
        ...CALL_ONLY_WORKSPACE_DEFINITIONS,
      ]
    }
    ...
    return ALL_BU_WORKSPACE_SLUGS.filter((slug) => allowedBuWorkspaces.includes(slug)).map((slug) => {
      ...
      const c = CALL_ONLY_WORKSPACE_DEFINITIONS.find((x) => x.workspace === slug)
      return { workspace: slug, label: d?.label ?? c?.label ?? slug.toUpperCase() }
    })
  }, [allowedBuWorkspaces])
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className={cn(
          'inline-flex items-center gap-2 pl-2.5 pr-2 py-1.5 rounded-lg text-xs font-semibold transition-colors border',
          isDark
      ...
      >
        <svg
          className={cn('w-3.5 h-3.5 shrink-0', isDark ? 'text-slate-400' : 'text-slate-500')}
          fill="none"
          stroke="currentColor"
          strokeWidth={2}
          viewBox="0 0 24 24"
        ...
        <svg
          className={cn('w-3.5 h-3.5 shrink-0 transition-transform', open && 'rotate-180', isDark ? 'text-slate-500' : 'text-slate-400')}
          fill="none"
          stroke="currentColor"
          strokeWidth={2}
          viewBox="0 0 24 24"
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-6">
        {NAV_SECTIONS.map((section) => {
          const visibleItems = section.items.filter((item) => {
            ...
            if (item.hideForCallsOnly && isCallsOnlyWorkspace(workspace)) return false
              ...
              <ul className="space-y-0.5">
                {visibleItems.map((item) => {
                  ...
                  return (
                    <li key={item.href}>
                      <Link
                        href={withWorkspaceQuery(item.href, workspace)}
      ...
      <div className={cn(
        ...
        <header className={cn(
          ...
          <div className="flex items-center gap-2">
            {(canViewWhatsApp || canViewCallReview) && <WorkspaceDropdown isDark={isDark} />}
            <button
              onClick={toggleTheme}
              className={cn(
