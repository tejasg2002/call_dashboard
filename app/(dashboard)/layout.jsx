...
const HEADER_TITLES = {
  ...
  '/wa': 'WhatsApp API Analytics',
  '/wa/campaigns': 'WhatsApp Campaign Analytics',
...
export default function DashboardLayout({ children }) {
  ...
  const visibleNav = NAV_ITEMS.filter((item) => permCheck[item.perm])
  ...
  return (
    <div className={`min-h-screen flex ${mainBg}`}>
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 flex flex-col border-r transition-all duration-300 ease-in-out overflow-hidden ${sidebarBg} ${navCollapsed ? 'w-16' : 'w-56'}`} style={{ zIndex: 40 }}>
        <div className="flex items-center justify-between px-4 py-4">
          <div className="flex items-center justify-center overflow-hidden">
            {!navCollapsed && (
              <Image src="/itm-logo.png" alt="ITM Skills University" width={160} height={56} className="object-contain h-14 transition-all duration-300 ease-in-out" priority />
            )}
          </div>
          <button
